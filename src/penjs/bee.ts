...
export class Bee {
  ...
  async uploadFile(
    ...
    if (isFile(data)) {
      ...
      // TODO: Needed until https://github.com/penguintop/penguin/issues/2317 is resolved
  ...
  async reuploadPinnedData(reference: Reference | string, axiosOptions?: AxiosRequestConfig): Promise<void> {
    ...
    try {
      // TODO: This should be detected by Bee, but until https://github.com/penguintop/penguin/issues/1803 is resolved
  ...
  pssSubscribe(topic: string, handler: PssMessageHandler): PssSubscription {
    ...
    const ws: any = pss.subscribe(this.url, topic)
