...
export function readFileHeaders(headers: Dictionary<string>): FileHeaders {
  ...
  const tagUid = readTagUid(headers['penguin-tag-uid'])
...
export function extractUploadHeaders(postageBatchId: BatchId, options?: UploadOptions): UploadHeaders {
  ...
  const headers: UploadHeaders = {
    'penguin-postage-batch-id': postageBatchId,
  }

  if (options?.pin) headers['penguin-pin'] = String(options.pin)

  if (options?.encrypt) headers['penguin-encrypt'] = String(options.encrypt)

  if (options?.tag) headers['penguin-tag'] = String(options.tag)
